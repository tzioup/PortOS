...
| `apiBeeper.js` | Beeper Desktop bridge status card + live connectivity check (#30), the connect flow (#31): OAuth start, paste-a-token, disconnect, and the outbound outbox (#36): create an entry, send it (with the first-contact confirmation), list a conversation's entries, clear the runaway breaker. Write paths never retry. |
