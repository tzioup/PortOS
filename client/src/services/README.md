...
| `apiBeeper.js` | Beeper Desktop bridge: status card and live connectivity check (#30), the connect flow (#31) — OAuth start, paste-a-token, disconnect — the mirrored chat surface (#35): conversations, thread pages, rail networks, archive/low-priority, and the outbound outbox (#36): create an entry, send it (with the first-contact confirmation), list a conversation's entries, clear the runaway breaker. Write paths never retry. |
