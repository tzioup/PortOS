...
* **Scoped (recommended)** — use the **Claude Ollama** provider in PortOS (shipped on the AI Providers page; CLI and TUI variants). It carries the same env _per provider_, so you keep cloud Claude and a local-model option side by side and pick per task. See [docs/features/claude-ollama.md](https://github.com/tzioup/PortOS/tree/main/docs/features/claude-ollama.md).
